...
def main(path, file):
    ...
    # 21 的抽取指标
    df21 = df[PARAM21]
    df21.to_excel(pd[:-5] + '_21版本_有缺失.xlsx', sheet_name='21')
    # 46 的抽取
    df46 = df[PARAM46]
    df46.to_excel(pd[:-5] + '_46版本_有缺失.xlsx', sheet_name='46')

    # 均值填充
    ndf = df.fillna(df.mean())
    ndf.to_excel(pd[:-5] + '_100版本_均值填充.xlsx')
    # 21 的抽取指标
    df21 = ndf[PARAM21]
    df21.to_excel(pd[:-5] + '_21版本_均值填充.xlsx', sheet_name='21')
    # 46 的抽取
    df46 = ndf[PARAM46]
    df46.to_excel(pd[:-5] + '_46版本_均值填充.xlsx', sheet_name='46')
    return None
