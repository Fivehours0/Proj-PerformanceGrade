...
if __name__ == '__main__':
    ...
    # 聚类
    X = df_scaled.iloc[:, [0, 1]]  # 取出 利用率,燃料比
    ...
    for j in range(N_CLUSTERS):
        ...
        temp = origin.groupby('label').get_group(j)
        plt.scatter(temp.iloc[:, 0], temp.iloc[:, 1], c=COLORS[j], label=j)

    plt.xlabel(origin.columns[0])
    plt.ylabel(origin.columns[1])
    plt.title(origin.columns[0] + "与" + origin.columns[1] + "2D聚类散点图")
    plt.legend()

    if save_fig:
        plt.savefig('C:/Users/Administrator/Desktop/figs/' + origin.columns[0] + "与" + origin.columns[1]
                    + "2D聚类散点图.png")
        plt.close()
