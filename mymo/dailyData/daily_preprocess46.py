...
def get_coke(input_table):
    ...
    df = get_df_format(param_list[0], input_table)
...
def get_molten_iron(input_table):
    param_list = [
        '[C]',
        '[Ti]',
        '[Si]',
...
def get_deltaTi(input_table):
    res = pd.DataFrame(data=None,
                       index=pd.date_range(start='2019-10-01', end='2019-11-30'))
    param = '[Ti]'
    df = get_df_format(param, input_table)
    # df['业务处理时间'] = pd.to_datetime(df['业务处理时间'])
    # df['采集项值'] = pd.to_numeric(df['采集项值'])
...
def get_delta_R2(input_table):
    # 计算delta_R2
    res = pd.DataFrame(data=None,
                       index=pd.date_range(start='2019-10-01', end='2019-11-30'))

    df = get_df_format('(CaO)', input_table)
...
def get_ratio(input_table):
    res = pd.DataFrame()

    ## 日产量
    df = get_df_format('受铁重量', input_table)
    # df['业务处理时间'] = pd.to_datetime(df['业务处理时间'])
    # df['采集项值'] = pd.to_numeric(df['采集项值'])
    res['日产量'] = df.groupby('业务处理时间').sum()['采集项值']

    ### 焦比
    df = get_df_format('冶金焦（自产）', input_table)
