...
Sub main2()
    Dim MyCtrl()
    MyCtrl = Array("热风温度", "送风风量", "富氧量", "喷吹速率", "球团矿比例", "焦炭负荷")
    
    Dim MyProd()
    MyProd = Array("热风压力", "透气性指数", "鼓风动能", "理论燃烧温度", "煤气利用率", "炉顶温度极差", "炉喉温度极差", "炉腹煤气发生量", "铁水温度", "[Ti]", "[Si]")
    
    FilePath = "C:\Users\Administrator\Desktop\新时滞报告\figs\" '获取工作路径,方便读取png图片
...
' 原始变化
    ...
    Selection.TypeText Text:="根据所采集的数据，" + prod + "与" + ctrl + "的原始变化曲线如图" + Str(picIdx) + ".1所示。为方便对比，对两指标参数的数值进行了标准化处理。"
    Selection.Style = ActiveDocument.Styles("正文样式")
    If (prod <> "焦炭负荷" And prod <> "铁水温度" And ctrl <> "球团矿比例") Then
        Selection.TypeText Text:="另外，为了更好地体现时滞处理的效果，我们选取了某月中两天的变化趋势进行比较。"
        Selection.Style = ActiveDocument.Styles("正文样式")
    End If
    Selection.TypeParagraph
    
    Selection.InlineShapes.AddPicture FileName:=FilePath + prod + "与" + ctrl + "的波动图(未滞后处理).png", LinkToFile:=False, SaveWithDocument:=True
    Selection.ParagraphFormat.Alignment = wdAlignParagraphCenter '居中
    Selection.TypeParagraph
...
'                           ".2所示，结合时间滞后范围参数设定，最终确定" + prod + "相对于" + ctrl + "的滞后时间为" + Str(time(j * 9 + i)) + "分钟。"
    ...
    Selection.InlineShapes.AddPicture FileName:= _
         FilePath + prod + "与" + ctrl + "的滞后时间分析图.png", _
         LinkToFile:=False, SaveWithDocument:=True
        Selection.ParagraphFormat.Alignment = wdAlignParagraphCenter '居中
    Selection.TypeParagraph
...
'(3)根据滞后时间进行处理后的曲线
   ...
   ' Selection.TypeText Text:="根据所获得的两个变量的时滞分析结果，对两个原始曲线进行调整，即将生产指标参数" + prod + "往前调整" + Str(time(j * 9 + i)) + _
    ...
    Selection.InlineShapes.AddPicture FileName:= _
        FilePath + prod + "与" + ctrl + "的波动图(滞后处理后).png", _
         LinkToFile:=False, SaveWithDocument:=True
        Selection.ParagraphFormat.Alignment = wdAlignParagraphCenter '居中
    Selection.TypeParagraph
